...
// Todo: blockConfirmations should be cleaned up periodically to prevent memory leak
type BlockProcessor struct {
    confirmedBlocks    []Block
    confirmedHeight    uint64
    blockConfirmations map[uint64]map[Block]int // height -> block -> count
    peerNodes          []Peer
...
func (p *BlockProcessor) GetBlocks() ([]Block, uint64) {
    ...
    return slices.Clone(p.confirmedBlocks), p.confirmedHeight
}
...
func (p *BlockProcessor) Initialize() error {
    ...
    for i, peer := range p.peerNodes {
        ...
        go func(i int, peer Peer) {
            defer wg.Done()
            // Todo: use a timeout context to prevent hanging
